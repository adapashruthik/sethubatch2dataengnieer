...
try:
    ...
    if len(sys.argv) == 1:
        ...
        nums = [eval(x) for x in sys.argv[1:]]
        largest = nums[0]
        for n in nums[1:]:
            if n > largest:
                largest = n
...
print(a [: -4 : -1])		# oar
print(a [-3 : -1])		# Ra
print(a [-3 : ])		# a[-3 : -1 : 1]  --->  string  from  indexes  -3  to  -2  in  steps  of  1  i.e.  Ra
print(a [ : : ])		# Rao
print(a [ : ])			# Rama<space>Rao
print(a [ : : -1])		# Rama<space>Rao
print(a [ : : -2])		# oaR<space>amaR
...
String   should  have  alternate  character  and  digit	a[i]  is  ith  char  of  string  and  a[i + 1]  is  (i + 1)th  char  of  string
'''
