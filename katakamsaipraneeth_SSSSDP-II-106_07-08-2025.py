...
try:
    a = [eval(x) for x in argv[1:]]
    print("sort  command  line  inputs  in  ascending  order:", sorted(a))
    print("sort  command  line  inputs  in  descending  order:", sorted(a, reverse=True))
except ValueError:
    print("Pls  don't  send  number  and  string  inputs  together")
except NameError:
    print("Pls  don't  send  string  inputs") 
...
print(a [: -4 : -1]) # a[0:-4:-1]--> empty string
...
print(a [ : -6 : -1]) # a[0:-6:-1]-->empty string
print(a [2 : -3]) # a[2:-3:1]-->ma 
print(a [1 : 6 : 2]) # a[1:6:2]-->aaR
print(a [ : -5 : -5]) # empty string
print(a [2 : -5]) # a[2:-5:1]-->m
print(a [2 : -5 : 2]) # a[2:-5:2]-->m
print(a [ : 0 : -1]) # a[0:0:-1]-->empty string
print(a [-5 : 0 : -2]) # a[-5:0:-2]-->a
...
elif not all(s[i].isalpha() and s[i+1].isdigit() for i in range(0, len(s), 2)):
    print("String should have alternate character and digit")
else:
...
for i in range(0, len(a), 2):
    char_part = a[i]               
    num_part = int(a[i + 1])       
    b += char_part + chr(ord(char_part) + num_part)
