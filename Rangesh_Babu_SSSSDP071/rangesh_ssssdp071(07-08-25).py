...
1)Write  a  program  to  determine  largest  command  line  input 
    ...
    What  is  the  issue  with  max(argv[1:])) ?  --->Largest  string  is  obtained  but  not  
largest  number 
2) py  prog2.py 
...
try: 
    # Convert all arguments (except script name) to float 
    ...
    try: 
        ...
        nums = any(x.replace('.', '', 1).isdigit() for x in argv[1:]) 
        strs = any(not x.replace('.', '', 1).isdigit() for x in argv[1:]) 
        if nums and strs: 
...
2) Write  a  program  to  determine  command  line  input  is  even  number  or  odd  
number 
...
sys.exit() 
try: 
a = [float(x) if x.lower() not in ['true', 'false'] else bool(x.lower() == 'true') for x in 
sys.argv[1:]] 
if not all(isinstance(x, (int, float, bool)) for x in a): 
print("Pls send number inputs") 
sys.exit() 
print("argv:", sys.argv) 
print("a:", a) 
...
print("Pls send number inputs") 
4)Write  a  program  to  sort  command  line  inputs  in  ascending  order  and  
descending  order 
...
try: 
    ...
    nums = any(x.replace('.', '', 1).isdigit() for x in sys.argv[1:]) 
    strs = any(not x.replace('.', '', 1).isdigit() for x in sys.argv[1:]) 
    if nums and strs: 
...
print(a [-6 : ]) #ma Rao 
print(a [: -4 : -1]) #oaR 
print(a [-3 : -1]) #  a[-3 : -1 : 1]  --->  string  from  indexes  -3  to  -2  in  steps  of  1  i.e.  
Ra 
print(a [-3 : ]) #Rao 
print(a [ : : ]) #Rama Rao 
print(a [ : ]) #Rama Rao 
print(a [ : : -1]) #oaR amaR 
print(a [ : : -2]) #oRaa 
print(a [ -2 : : -2])  #  a[-2 : -9 : -2]  --->  string  from  indexes  -2  to  -8  in  steps  of  -2  
i.e.  a<space>mR 
print(a [2 : 8]) #ma Rao 
...
print(a [ : 0 : -1]) #oaR ama 
print(a [-5 : 0 : -2]) #aa 
5)Write  a  program  to  concatenate  two  strings  separated  by  space  but  swap  first  
two 
...
if len(s1)>4 : 
    ...
    print("") 
7)Write  a  program  to  print  characters  of  the  string  in  forward  and  reverse 
directions  without  slice 
                0      1     2      3     4 
Let  input  be    V     A     M     S     I 
    -5    -4    -3    -2    -1 
What  are  the  outputs ?  --->                                                                      Character  at  
index  0  :  V 
...
for i in range(len(s1)): 
    print(f"character at index {i}: {s1[i]}") 
 
for i in range(-1,-len(s1)-1,-1): 
    print(f"character at index {i}: {s1[i]}") 
 
8)Write  a  program  to  print  characters  at  even  and  odd  indexes  without  slice 
 
                                0      1      2      3     4     5     6      7 
Let  input  be        R      a     m      a             R     a      o 
odd =  '' + 'a' + 'a' + 'R' + 'o' = 'aaRo' 
even =   '' + 'R' + 'm' + ' '  + 'a' = 'Rm a' 
1) What  action  to  be  made  when  index  is  even ?  --->Concatenate  the  character  
to  even  object 
2) What  action  to  be  made  when  index  is  odd ?  --->Concatenate  the  characeter  
to  odd  object 
...
'AAAABBBCC$$$$$' 
   ------------------------------------------------------- 
What  is  the  difference  between  a[i]   and  a[i + 1] ?  --->a[i]  is  ith  char  of  string  
and  a[i + 1]  is  (i + 1)th  char  of  string 
Code: 
try: 
    s1=input("Enter  any  string  with  alternate  character  and  digit : ") 
    for i in range(0,len(s1),2): 
        print(s1[i]*int(s1[i+1]),end="") 
except: 
    print("String should have alternate character and digit") 
10)Write  a  program  to  merge  two  strings  to  form  a  new  string 
...
11) Write  a  program  to  remove  duplicate  characters  of  the  string  without  using  
set 
1) Let  input  be   RAMA  RAO 
   What  is  the  output ? ---> RAM<space>O 
2) out = '' + 'R' = 'R' + 'A' = 'RA' + 'M' = 'RAM' + ' ' = 'RAM ' + 'O' = 'RAM O' 
3) What  action  to  be  made  if  the  character  is  not  in  out  object ?  --->Concatenate  
the  character  to  out  object 
4) What  action  to  be  made  if  the  character  is  already  in  out  object ?  --->Ignore  
the  character 
...
print('Sec'. len()) #error:no argument in len() 
...
What  is  the  output ?  --->  AEMPZ_DF 
 ...
 A    4     M    3    Z    5    D     2 
  ...
  6       'D'           '2'          'AEMPZ_' + 'D' + chr(68 + 2) = 'AEMPZ_' + 'D' + 'F' = 
'AEMPZ_DF' ----------------------------------------------------------------------------------- 
