...
5) py  prog3.py  Ten
    What  is  the  output  ?  --->  Pls  send   an integerÂ Â input
'''
from sys import argv
try:
	for x in argv[1:]:
...
try:
	...
	print(f'Descending Order : {sorted(a , reverse = True)}')	
except TypeError:
	print('Please do not send number and string inputs together')
...
print(a [2 : 4]) # a[2:4:1] --> string from indexes 2 to 2 in steps of 1 i.e. ma 
print(a [2 : ]) # a[2 : 8 : 1] --> string from indexes 2 to 7 in steps of 1 i.e. ama<space>Ra
...
print(a [ : ]) # a[0 : 8 : 1]  --> string from indexes 0 to 7 in steps of 1 i.e. Rama<space>Rao 
print(a [ : : -1]) # a[-1 : -9 : -1] ---> string from indexes -1 to -9 in steps of -1 i.e. oaR<space>amaR
print(a [ : : -2]) # a[-1 : -9 : -2]   ---> string from indexes -1 to -9 in steps of -2 i.e. oRaa
print(a [ -2 : : -2])  #  a[-2 : -9 : -2]  --->  string  from  indexes  -2  to  -8  in  steps  of  -2  i.e.  a<space>mR
print(a [2 : 8]) # a[2 : 8 : 1] --->  string from indexes 2 to 7 in steps of 1 i.e. ama<space>Ra
print(a [2 : 8 : -1]) a[2 : 8 : -1] --> ''
print(a [ : -6 : -1]) # a[0 : -6 : -1] ---> ''
print(a [2 : -3]) # a[2 : -3 : 1] --> string from indexes 2 to -4 in steps of 1 i.e. ama<space>
print(a [1 : 6 : 2]) # a[1 : 6 : 2] --> string from indexes 1 to 5 in steps of 1 i.e. ama<space>Ra
print(a [ : -5 : -5]) # a[-1 : -5 : -5] --> o
print(a [2 : -5]) # a[2 : -5 : 1] ---> m
print(a [2 : -5 : 2]) # m
print(a [ : 0 : -1]) # a[-1 : 0 : -1] ---> oaR<space>ama
...
for y in range(1 , len(a) + 1):
	print(f'Character at index -{y} : {a[-y]}')
...
print(len(' ')) # 1
print(len('A2#')) # 3
print(len(3456)) # error as len() argument must be sequence
print('Sec'. len()) # error
...
print(chr(36)) # Converts  unicode  value  32  to  '$'
print(chr(32)) # Converts  unicode  value  36  to  ' '
