# eval()  function  demo  program
print(eval('25'))                          # 25
print(eval('10.8'))                        #10.8
print(eval('False'))                       #False
print(eval('3+4j'))                        # 3+4j
print(eval('Hyd'))                         #error
print(eval("    'Hyd'   "))                #Hyd
print(eval('3 + 4 * 5'))                   #23
print(eval('[10 , 20 , 15 , 18]'))         #[10,20,15,18]
print(eval('{10,20,15,18,20,12,18}'))      #{10,20,15,18,12}
print(eval('(10 , 20 , 30)'))              #(10,20,30)
print(eval("{10 : 'Hyd' , 10 : 'Sec'}"))   #{10:'Sec'}
print(eval(4 + 5))                         #error argument must be in string form
print()
#  Tricky  program
# Find  outputs  (Home  work)
print(eval("    'hyd'   "))      #hyd
hyd = 'Sec'                      #Assigning string object sec to reference hyd
print(eval('hyd'))               #sec
sec = '25'                       # Assigning string object 25 to reference sec
print(eval('sec'))               #25
print(eval(sec))                 #25
cyb = 10.8                       #Assigning float object 10.8 to reference cyb
print(eval('cyb'))               # 10.8
#print(eval(cyb))                #error because In eval argument must be in string 
print()

#  Tricky  program
#  Find  output  (Home  work)
print(eval('print("Hyd")'))  	 # Hyd <next line> None
print()

#  Find  outputs  (Home  work)
print(bool('False'))      	#True
print(eval('False'))      	#False
print(bool(''))           	#False
print(eval('  ""  '))     	#Empty string
print(eval(''))           	#syntax error
print(eval('  " "   '))   	#<space>string
#print(eval(' '))          	#Error syntax error
print()

# What  is  the  advantage  of  eval(input()) ?
x = eval(input('Enter  any  input  :  '))  #Takes input from keyboard assigns to object x
print(type(x))   #<class 'object'>
print(x)         # user given string object      
print() 

# What  is  a  better  approach  to  read  string  input ?
a = input('Enter  any  string  :  ') 
print(len(a))  #length of the given string
print(a)       #user given string 
b = eval(input('Enter   any  string  : '))
print(len(b))  #length of the given object
print(b)       #user given object
print()

# sep  argument  demo  program  (Home  work)
a , b , c = 25 , 10.8 , 'Hyd'
print(a , b , c , sep = ',')   #   25 , 10.8 , Hyd
print(a , b , c , sep = '\t')  #   25<tab>10.8<tab>Hyd<tab>
print(a , b , c , sep = '---') #   25---10.8---Hyd
print(a , b , c , sep = '\n')  #   25<next line>10.8<next line>Hyd<next line>
print(a , b , c)               #   25 10.8 Hyd
print(a , b , c , separator = ':') #error sep cannot be as separator 
print()

# Find  outputs  (Home  work)
a , b , c = 25 , 10.8 , 'Hyd'
print(a , b , c , end = '---')                  # 25 10.8 Hyd---<same line>
print(a , b , c , sep = ',,,')                  #25,,,10.8,,,Hyd
print(a , b , c , sep = ':::' , end = '\t\t\t') # 25:::10.8:::Hyd:::<tab><tab><tab>
print(a , b , c)         		        #25 10.8 Hyd
print()

# Find outputs  (Home  work)
print('Hyd')   #Hyd
print()        #Empty <next line>
print('Sec')   #Sec
print()        #Empty <next line>
print('Cyb')   # Cyb
print()        # Empty <next line>

# Find  outputs  (Home  work)
l = [10 , 20 , 30 , 40]
t = (10 , 20 , 30 , 40)
s = {10 , 20 , 30 , 40}
print(l , t , s) #[10,20,30,40] (10,20,30,40) {10,20,30,40}
print()

#  Find  outputs (Home  work)
a = 25
b = '%f'  %a 			#Converting int 25 to string float object 
print(b)     			#25.000000
print(type(b)) 			#<class 'str'>
x = 10.8
y = '%d'  %x 			#converting float 10.8 to string int object '10'
print(y) 			#10
print(type(y)) 			#<class 'str'>
m = [10 , 20 , 15 , 18]
n = '%s'  %m			#list to string and assigning to n
print(n)			#'[10,20,15,18]'
print(type(n))			#<class 'str'>
print()

# Find  Outputs  (Home  work)
a = 10.9274
print('%8.2f'  %a)  	#<3  spaces>10.93
print('%9.1f'  %a)	#<5 spaces>10.9
print('%10.3f'  %a)	#<4 spaces>10.927
print('%.2f'  %a)	#10.93
print('%.6f'  %a)	#10.927400
print('%f'  %a)		#10.927400
print()

# Find  outputs (Home  work)
a = 'Hyd'
print('%7s'  %a)   	#<4  spaces>Hyd
print('%-7s'  %a)  	#Hyd<4   spaces>
print('%2s'  %a)   	#Hyd  and  ignores  smaller width
print('%s'  %a)		#Hyd
print('%s' , a)		#%s Hyd
print('%s'  a)		#error
print('%s' , %a)	#error
print(a)		#Hyd
print()

# Find  outputs  (Home  work)
a = [10 , 20 , 30 , 40]
print('%s'  %a)		#[10,20,30,40]
print('%s' , a)		#%s [10,20,30,40]
print('%s'  a)		#error
print('%s' , %a)	#error
print('%l'  %a)		#error
print(a)		#[10,20,30,40]
print()

#Find outputs  (Home  work)
a = 25
b = 10.9274
c = 'Hyd'
print('%d    %f    %s'  %(a , b , c))		#25 10.927400 Hyd
print('%i    %g    %s'    %(a , b , c))		#25 10.9274 Hyd
print('%s    %s    %s'  %(a , b , c))		#25 10.9274 Hyd
print('%d    %g    %s'  , a , b , c)		#%d %g %s 25 10.9274 Hyd
print('%d    %g      %s'   a , b , c)		#error
print('%d    %g    %s'  ,  %(a , b , c))	#error
print('%d    %g    %s'    %a%b%c)		#error
print('%d'    %a  ,  '%f'     %b  ,  '%s'   %c) #25 10.927400 Hyd